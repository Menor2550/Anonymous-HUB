...
createToggle(homePage, "Fly", false, function(s)
	if s then
		startFly()
	else
		stopFly()
...
createSlider(homePage, "Fly Speed", 10, 200, 50, function(val)
	flySpeed = val
end)
...
flyKeyFrame.BackgroundColor3 = Color3.fromRGB(36, 36, 36)
flyKeyFrame.BorderSizePixel = 0
flyKeyFrame.Parent = homePage
...
flyKeyLabel.Size = UDim2.new(1, -80, 1, 0)
flyKeyLabel.Position = UDim2.new(0, 12, 0, 0)
flyKeyLabel.Text = "Fly Key"
flyKeyLabel.Font = Enum.Font.GothamMedium
flyKeyLabel.TextSize = 13
flyKeyLabel.TextColor3 = Color3.fromRGB(210, 210, 210)
flyKeyLabel.TextXAlignment = Enum.TextXAlignment.Left
flyKeyLabel.Parent = flyKeyFrame

local flyKeyBtn = Instance.new("TextButton")
flyKeyBtn.Size = UDim2.new(0, 60, 0, 22)
flyKeyBtn.Position = UDim2.new(1, -70, 0.5, -11)
...
flyKeyBtn.TextColor3 = Color3.fromRGB(0, 255, 170)
flyKeyBtn.AutoButtonColor = false
flyKeyBtn.Parent = flyKeyFrame
Instance.new("UICorner", flyKeyBtn).CornerRadius = UDim.new(0, 4)

flyKeyBtn.MouseButton1Click:Connect(function()
	if listeningForKey then return end
	listeningForKey = true
	flyKeyBtn.Text = "..."
	flyKeyBtn.BackgroundColor3 = Color3.fromRGB(0, 170, 120)
	local conn
	conn = UserInputService.InputBegan:Connect(function(input, gpe)
		if gpe then return end
		if input.KeyCode == Enum.KeyCode.Unknown then return end
		flyKey = input.KeyCode
		flyKeyBtn.Text = string.upper(input.KeyCode.Name)
		flyKeyBtn.BackgroundColor3 = Color3.fromRGB(50, 50, 50)
		listeningForKey = false
		conn:Disconnect()
	end)
...
UserInputService.InputBegan:Connect(function(input, gpe)
	...
	elseif input.KeyCode == flyKey and not listeningForKey then
		if flyEnabled then
			stopFly()
		else
			startFly()
